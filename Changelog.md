...
All notable changes to the LANi-Platform will be documented in this file.
...
### Added
- Разработена е цялостна **криптографска лицензионна система (LANi License Manager)** в `app/services/license_manager.py` с поддръжка за 1-годишни търговски подписи, следене на устройствови лимити и демо-режим за сигурност.
- Вградени са REST API точки за проверка, качване и управление на лицензи в `app/api/settings.py`.
...
- Създаден е инструмент за генериране на криптирани лицензи `scripts/generate_license.py` и детайлно ръководство `docs/licensing/Licensing_Guide.md` на български език.
- Добавен е нов пакет за автоматизирани интеграционни и модулни тестове на лицензионния мениджър (`tests/test_license_manager.py` и `tests/test_license_integration.py`) с 100% успеваемост.
